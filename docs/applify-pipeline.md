...
- **Profile editing uses explicit Save / Discard per section instead of save-on-blur.** Required fields (experience title and company; education institution plus degree or field; certification and project names) are validated client-side and again by `PATCH /profile`, which returns a structured 422 instead of silently dropping incomplete entries. Fully blank rows are removed before saving. Unsaved changes are guarded on in-app navigation and page unload.
